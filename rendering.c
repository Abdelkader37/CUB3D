...
uint32_t get_texture_pixel(mlx_image_t *texture, uint32_t x, uint32_t y)
{
    ...
    uint32_t r = pixels[index];
    uint32_t g = pixels[index + 1];
    uint32_t b = pixels[index + 2];
    uint32_t a = pixels[index + 3];
...
void	wall(t_mlx_data *data, int x)
{
    ...
    {
        int tex_y = ((y - data->draw_start) * texture->height) / data->line_height;
